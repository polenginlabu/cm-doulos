...
namespace App\Filament\Resources\SuynlEnrollmentResource\RelationManagers;
...
class AttendancesRelationManager extends RelationManager
{
    ...
    protected static ?string $title = 'Lesson Attendance';
        ...
        return $form
            ->schema([
                Forms\Components\TextInput::make('lesson_number')
                    ...
                    ->minValue(1)
                    ->maxValue(10)
                    ->helperText('Lesson number (1 to 10)')
                    ->rules([
                        function ($get, $livewire) {
                            return function (string $attribute, $value, \Closure $fail) use ($get, $livewire) {
                                if ($value && $livewire->ownerRecord) {
                                    ...
                                    $query = \App\Models\SuynlAttendance::where('suynl_enrollment_id', $livewire->ownerRecord->id)
