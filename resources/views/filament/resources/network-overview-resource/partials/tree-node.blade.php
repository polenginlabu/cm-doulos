<li>
    @php
        $borderColor = ($node['is_primary_leader'] ?? false) ? 'primary-leader' :
                      (($node['is_network_admin'] ?? false) ? 'network-admin' :
                      (($node['is_equipping_admin'] ?? false) ? 'equipping-admin' : 'member'));
    @endphp
        ...
        <div class="flex flex-col h-full">
            <div class="flex items-start gap-3 mb-3">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    @elseif(isset($node['is_equipping_admin']) && $node['is_equipping_admin'])
                        <span class="inline-block px-2 py-0.5 text-xs font-medium rounded-full bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300">
                            Equipping Admin
                        </span>
                    @else
            ...
            @endif
            @if(isset($node['has_children']) && $node['has_children'])
